...
let lastApiReachabilityCheck = 0;
let lastApiReachabilityResult = false;
...
async function isApiServerReachable() {
    ...
    const now = Date.now();
    if (lastApiReachabilityResult && (now - lastApiReachabilityCheck < 10000)) {
        return true;
    }
    const targetUrl = `${getApiBaseUrl()}/api/auth.php?action=ping`;
    try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 8000); // 8-second timeout for free hosting latency
        ...
        if (!response.ok) {
            lastApiReachabilityResult = false;
            return false;
        }
        const data = await response.json();
        const isOk = data && data.status === 'success';
        lastApiReachabilityResult = isOk;
        lastApiReachabilityCheck = Date.now();
        return isOk;
    } catch (e) {
        lastApiReachabilityResult = false;
        return false;
...
async function fetchStudentsFromDatabase() {
    const isOnline = await isApiServerReachable();
    console.log("[DB Sync] Admin data sync started. API reachable:", isOnline);
    if (isOnline) {
        try {
            const session = getSession();
            if (session && session.role === 'admin') {
                const timestamp = new Date().getTime();
                const response = await fetch(`${getApiBaseUrl()}/api/admin.php?action=get_all_data&t=${timestamp}`, {
                    method: 'GET',
                    cache: 'no-store'
                });
                const data = await response.json();
                console.log("[DB Sync] Admin API Response:", data);
                if (data.status === 'success') {
                    ...
                    if (Array.isArray(data.certificates)) {
                        ...
                        // Populate a global completedCourses based on course certificates
                        const courseCompletions = data.certificates.filter(c => c.type === 'course');
                        localStorage.setItem('completedCourses', JSON.stringify(courseCompletions));
                    }
                    if (Array.isArray(data.quizScores)) {
                        localStorage.setItem('quizScores', JSON.stringify(data.quizScores));
                    }
                    if (Array.isArray(data.enrollments)) {
                        localStorage.setItem('adminEnrollments', JSON.stringify(data.enrollments));
                    }
                    console.log("[DB Sync] Admin data successfully synced to LocalStorage.");
                }
            } else {
            ...
            console.warn("[DB Sync] Failed to fetch admin data from database:", e);
        }
    } else {
    ...
    if (isOnline) {
        try {
            ...
            if (data.status === 'success' && Array.isArray(data.certificates)) {
                ...
                // Keep the existing completed-course records and merge certificates from DB.
                // IMPORTANT: preserve userEmail so the completion check does not treat an
                // already-completed course as a new completion on every page load.
                const completedCourses = JSON.parse(
                    localStorage.getItem(getUserStorageKey('completedCourses')) || '[]'
                );
                const userEmail = session.email || localStorage.getItem('userEmail') || '';

                certificates.forEach(cert => {
                    const exists = completedCourses.some(c =>
                        String(c.id) === String(cert.courseId) &&
                        (c.userEmail || userEmail) === userEmail
                    );
                ...
                localStorage.setItem(
                    getUserStorageKey('completedCourses'),
...
async function initializeApp() {
    // 1. Run UI/Navigation setup instantly to prevent FOUC (Flicker)
    ...
    // 2. Load DB data (takes time on live server)
    ...
    // 3. Clear old caches if necessary
    ...
    // 4. Render content that depends on DB
            ...
            <div class="section" style="text-align: center; padding: 1.5rem 0;">
                <div class="container" style="display: flex; justify-content: center;">
                    <div style="background: rgba(16,185,129,0.08); border: 2px solid #10b981; border-radius: 12px; padding: 1.5rem; width: 100%; max-width: 500px; text-align: center;">
                        ...
                        <a href="${getApiBaseUrl() === '.' ? 'student/' : ''}certificates.html" class="btn btn-primary" style="display: inline-block; padding: 0.7rem 1.6rem; font-size: 0.95rem; font-weight: 700; text-decoration: none; border-radius: 8px;">🎓 View Your Certificate</a>
                    </div>
                </div>
            </div>
        `;
...
function displayQuiz() {
    ...
    if (!document.getElementById('quizBreadcrumb')) {
        ...
        ]).replace('<div aria-label="breadcrumb"', '<div id="quizBreadcrumb" aria-label="breadcrumb"');
...
async function checkAndTriggerCourseCompletion(courseId) {
    ...
    // All conditions met! Mark as completed only ONCE.
    const currentEmail = session?.email || localStorage.getItem('userEmail') || '';
    ...
    const alreadyCompleted = completedCourses.some(c =>
        ...
        (c.userEmail || '') === currentEmail
    );

    // If the course is already completed, do NOTHING.
    // This prevents the certificate notification from appearing again
    // when another page is opened or the page is refreshed.
    if (alreadyCompleted) {
        return false;
    }
    ...
    // Generate the Course Completion Certificate only once.
    await generateCertificate(parseInt(courseId), course.title, 'course');

    launchConfetti();
    showCertificateNotification(
        `Congratulations! You completed ${course.title} and earned your Certificate of Achievement! 📜`
    );
...
window.markVideoAsWatched = async function(courseId) {
    ...
    if (!watched.includes(String(courseId))) {
        ...
        // If completion happened now, do not reload immediately; otherwise the
        // 1-minute certificate notification would disappear with the page reload.
        const courseJustCompleted = await checkAndTriggerCourseCompletion(courseId);
...
function downloadCertificate(certificate) {
    ...
    const originalEl = document.querySelector('.certificate-display');
    if (!originalEl) {
    ...
    if (typeof html2canvas === 'undefined') {
        ...
        script.onload = () => generateDownloadWithHtml2Canvas(originalEl, certificate);
        document.head.appendChild(script);
    } else {
        generateDownloadWithHtml2Canvas(originalEl, certificate);
    }
}

function generateDownloadWithHtml2Canvas(originalEl, certificate) {
    // Create a clone to render without mobile scaling
    ...
    clone.style.padding = '3.5rem 4rem';
    
    // Strip border and shadows from clone to draw them manually with high quality
    clone.style.border = 'none';
    clone.style.boxShadow = 'none';
    const corners = clone.querySelectorAll('.certificate-corner');
    corners.forEach(c => c.style.display = 'none');
    
    // Some browsers need the clone to be in the DOM
    ...
    html2canvas(clone, { 
        ...
        // Draw the outer green border (12px * 2 = 24px)
        ctx.strokeStyle = '#065f46';
        ctx.lineWidth = 24;
        ctx.strokeRect(12, 12, width - 24, height - 24);
        
        // Draw the gold inset box-shadow (inset 4px background + 6px gold * 2)
        // 24px (green border) + 8px (background padding) = 32px inset
        // 32px + 6px (half of 12px gold line) = 38px
        ctx.strokeStyle = '#d4af37';
        ctx.lineWidth = 12;
        ctx.strokeRect(38, 38, width - 76, height - 76);
        
        // Draw the 4 corners (40px * 2 = 80px, border 2px * 2 = 4px, inset 24px * 2 = 48px)
        ctx.strokeStyle = '#d4af37';
        ctx.lineWidth = 4;
        // Top Left
        ctx.beginPath(); ctx.moveTo(48 + 80, 48); ctx.lineTo(48, 48); ctx.lineTo(48, 48 + 80); ctx.stroke();
        // Top Right
        ctx.beginPath(); ctx.moveTo(width - 48 - 80, 48); ctx.lineTo(width - 48, 48); ctx.lineTo(width - 48, 48 + 80); ctx.stroke();
        // Bottom Left
        ctx.beginPath(); ctx.moveTo(48 + 80, height - 48); ctx.lineTo(48, height - 48); ctx.lineTo(48, height - 48 - 80); ctx.stroke();
        // Bottom Right
        ctx.beginPath(); ctx.moveTo(width - 48 - 80, height - 48); ctx.lineTo(width - 48, height - 48); ctx.lineTo(width - 48, height - 48 - 80); ctx.stroke();
...
function printCertificate(certificate) {
    ...
    // Calculate prefix based on path
    const path = window.location.pathname.replace(/\\/g, '/');
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                body { margin: 0; padding: 20px; font-family: Georgia, serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; background: #fff; }
                @media print {
                    @page { size: landscape; margin: 0; }
                    body { padding: 0; }
                    .certificate-display { -webkit-print-color-adjust: exact; print-color-adjust: exact; transform: scale(1) !important; margin: 0 !important; width: 1000px; }
                }
            </style>
        </head>
        <body>
...
function showCertificateNotification(message) {
    // Remove an existing certificate notification before creating a new one.
    const existing = document.getElementById('certificateCompletionNotification');
    if (existing) {
        existing.remove();
    }

    const notification = document.createElement('div');
    notification.id = 'certificateCompletionNotification';
    ...
    // Show the notification for exactly 1 minute.
    setTimeout(() => {
...
function renderBreadcrumb(items) {
    ...
    return `
        <div aria-label="breadcrumb" style="background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); position: relative; z-index: 10; padding: 1rem 0; font-size: 0.95rem; text-shadow: 0 1px 2px rgba(0,0,0,0.4);">
            <div class="container">
                ${items.map((item, i) => {
